...
export function getUserProfile(userId) {
  return async (dispatch) => {
    try {
      const { data } = await domain.get(`/api/users/profile/${userId}`);
...
export function deleteProfile(userId) {
  return async (dispatch, getState) => {
    try {
      ...
      const { data } = await domain.delete(`/api/users/profile/${userId}`, {
        headers: {
          Authorization: "Bearer " + getState().auth.user.token,
        },
      });
...
export function getUsersCount() {
  return async (dispatch, getState) => {
    try {
      const { data } = await domain.get(`/api/users/count`, {
        headers: {
          Authorization: "Bearer " + getState().auth.user.token,
        },
      });
...
export function getAllUsersProfile() {
  return async (dispatch, getState) => {
    try {
      const { data } = await domain.get(`/api/users/profile`, {
        headers: {
          Authorization: "Bearer " + getState().auth.user.token,
        },
      });
